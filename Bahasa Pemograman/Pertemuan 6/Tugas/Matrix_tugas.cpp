...
int main() {
    ...
    char pilihan;
    do {
        int hasil = 0; // Inisialisasi hasil
        cout << "Matriks yang telah diinput:" << endl;
        tampilkanMatriks(matriks);

        cout << "Pilihan Perhitungan (a: Jumlah, b: Rata-rata, c: Ganti Genap, d: Ganti Kelipatan 3, e: Transpose, q: Keluar): ";
        cin >> pilihan;

        switch (pilihan) {
            case 'a':
                hasil = hitungJumlahMatriks(matriks);
                cout << "Jumlah semua bilangan dalam matriks: " << hasil << endl;
                break;
            case 'b':
                hasil = hitungRataRataMatriks(matriks);
                cout << "Rata-rata semua bilangan dalam matriks: " << hasil << endl;
                break;
            case 'c':
